...
### DC-002 — Entry content size bounded by D1 row limits

- **Forced by:** EC-002 (D1 SQLite row size limits) + FR-001 (store knowledge entry)
...
### DC-003 — Bulk ingest must process asynchronously for large payloads

- **Forced by:** EC-001 (Workers CPU time limits) + EC-002 (D1 row/transaction limits) + FR-010 (bulk ingest)
- **Why inevitable:** Workers have ~30ms CPU budget per request. Parsing, chunking, embedding, and storing a large document exceeds this budget. Either process asynchronously or the request times out. Rejecting large ingests would violate FR-010.
...
- **Forced by:** EC-004 (Vectorize optional binding — "system must degrade gracefully without it") + FR-002 (hybrid retrieval)
...
- **Forced by:** EC-002 (D1 exposes SQLite subset — FTS5 not guaranteed in all environments) + FR-002 (search must work)
...
- **Forced by:** EC-001 (Workers are stateless V8 isolates — memory lost after each request) + FR-007 (undo via transaction log) + FR-008 (view transaction history)
...
- **Forced by:** EC-005 (MCP specification defines tool schemas, transport, auth flow)
...
- **Forced by:** EC-001 (Workers CPU/memory budget) + EC-002 (D1 query cost scales with result size)
