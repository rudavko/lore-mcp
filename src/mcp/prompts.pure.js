		...
		messages: [
			{
				...
				content: {
					...
					text: [
						...
						"",
						"4. Use `engine_check` action=`history` to inspect recent changes, action=`status` to assess instance health before or after retrieval work, action=`auto_updates_status` to inspect current auto-update inspection limits, and action=`enable_auto_updates` when you need the one-time admin workflow-install link.",
						"",
		...
		messages: [
			{
				...
				content: {
					...
					text: [
						...
						"",
						"7. Review the resulting audit trail with `engine_check` action=`history`, and use action=`auto_updates_status` if you need to inspect current auto-update inspection limits.",
					].join("\n"),
				},
			},
		],
