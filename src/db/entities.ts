...
export async function createEntity(db: D1Database, name: string): Promise<CanonicalEntity> {
	const id = ulid();
	const now = sqliteNow();
...
async function resolveAliasExact(db: D1Database, name: string): Promise<CanonicalEntity | null> {
...
export async function resolveAlias(db: D1Database, name: string): Promise<CanonicalEntity | null> {
...
export async function mergeEntities(
	...
	const keepEntity = await db
		.prepare(`SELECT * FROM canonical_entities WHERE id = ?`)
		.bind(keepId)
		.first();
	if (!keepEntity) throw KnowledgeError.notFound("Entity", keepId);

	const mergeEntity = await db
		.prepare(`SELECT * FROM canonical_entities WHERE id = ?`)
		.bind(mergeId)
		.first();
	...
	const { results: subjTriples } = await db
		.prepare(`SELECT id FROM triples WHERE subject = ? AND deleted_at IS NULL`)
		.bind(mergeName)
		.all();
	const { results: objTriples } = await db
	...
	await db.batch([
		...
		db.prepare(`DELETE FROM canonical_entities WHERE id = ?`).bind(mergeId),
	]);
